...
class SentenceClasscify:
    ...
    def creataModel(self, batchSize):
        ...
        #mlstm_cell = tf.nn.rnn_cell.MultiRNNCell([lstmCell] * 2, state_is_tuple=True)
        value, _ = tf.nn.dynamic_rnn(lstmCell, bilstmlast, dtype=tf.float32)
        ...
        self.__optimizer = tf.train.AdamOptimizer(learning_rate=0.005).minimize(self.__loss, name='optimizer')
    ...
    def prediction(self, model_path, text):
        ...
        while True:
            ...
            y = tf.argmax(y, 2)
            print x[0]
            print [tag_map[i] for i in y.eval()[0] ]
